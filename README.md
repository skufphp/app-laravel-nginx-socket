...
*   **Современный стек:**
    ...
    *   **Nginx 1.29** — оптимизированный конфиг для Laravel через FastCGI.
...
*   **Production Ready:** Отдельные compose-конфигурации для локального production-запуска и дальнейшей интеграции в CI/CD.
...
*   `make up` — Запустить проект в dev-режиме (на порту `8050` или `NGINX_PORT`).
*   `make up-prod` — Запустить проект в prod-режиме локально (на порту 8050 по умолчанию).
